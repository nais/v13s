...
func TestWorkloadResyncModule_ResyncTriggersUpdaterOnPartialFailure(t *testing.T) {
	t.Parallel()
	...
	querier.EXPECT().
		UpdateImageState(mock.Anything, mock.Anything).
		Return(int64(1), nil).
		Once()
	...
	imageState := "resync"
	_, err := module.Resync(context.Background(), Input{
		Cluster:       "c-1",
		Namespace:     "ns-1",
		Workload:      "wl-1",
		WorkloadType:  ptrString("app"),
