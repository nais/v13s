...
image_vulnerabilities AS (
    ...
    WHERE (sqlc.narg('include_suppressed')::BOOLEAN IS TRUE OR COALESCE(sv.suppressed, FALSE) = FALSE)
     AND (sqlc.narg('since')::timestamptz IS NULL OR rv.severity_since > sqlc.narg('since')::timestamptz)
     AND (sqlc.narg('severity')::INT IS NULL OR rv.severity = sqlc.narg('severity')::INT)
)
SELECT id,
       image_name,
       image_tag,
