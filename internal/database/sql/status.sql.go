...
paged_jobs AS (
    SELECT
        id, kind, state, metadata, errors, finalized_at, attempt
    FROM
        filtered_jobs
    ORDER BY
        id DESC
...
SELECT
    id, state, attempt, max_attempts, attempted_at, created_at, finalized_at, scheduled_at, priority, args, attempted_by, errors, kind, metadata, queue, tags, unique_key, unique_states
FROM
    river_job
WHERE (kind = 'add_workload'
    AND args -> 'Workload' ->> 'Name' = $1::TEXT
    AND args -> 'Workload' ->> 'WorkloadType' = $2::TEXT
    AND args -> 'Workload' ->> 'Namespace' = $3::TEXT
    AND args -> 'Workload' ->> 'Cluster' = $4::TEXT)
    OR (kind = 'get_attestation'
        AND args ->> 'ImageName' = $5::TEXT
        AND args ->> 'ImageTag' = $6::TEXT)
