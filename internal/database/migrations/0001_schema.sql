...
BEGIN
    IF EXISTS(
        ...
        FROM
            pg_catalog.pg_roles
        WHERE
            rolname = 'cloudsqlsuperuser') THEN
...
CREATE TYPE vulnerability_suppress_reason AS ENUM(
    'in_triage',
    'resolved',
    'false_positive',
    'not_affected',
...
ALTER TABLE workloads
    ADD CONSTRAINT fk_image FOREIGN KEY (image_name, image_tag) REFERENCES images(NAME, tag) ON DELETE CASCADE;
...
ALTER TABLE vulnerabilities
    ADD CONSTRAINT fk_cve FOREIGN KEY (cve_id) REFERENCES cve(cve_id) ON DELETE CASCADE;
