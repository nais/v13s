...
ALTER TABLE vuln_fix_summary
    DROP CONSTRAINT IF EXISTS vuln_fix_summary_pkey;
...
vuln_events_with_lag AS(
    SELECT
        *,
        LAG(count) OVER(PARTITION BY workload_id,
            severity ORDER BY snapshot_date) AS prev_count
FROM
    vuln_events
),
introduced AS(
    ...
    FROM
        vuln_events_with_lag
    WHERE
        count > 0
        AND(prev_count = 0
            OR prev_count IS NULL)
),
fixed AS(
    SELECT
        workload_id,
...
SELECT
    ...
    CASE WHEN MIN(f.fixed_at) IS NOT NULL
        AND MIN(f.fixed_at) <= for_date THEN
        TRUE
    ELSE
        FALSE
    END AS is_fixed,
...
FROM
    introduced i
    LEFT JOIN fixed f ON f.workload_id = i.workload_id
...
BEGIN
    ...
    SELECT
        ...
        u.introduced_at,
        u.fixed_at,
        CASE WHEN u.fixed_at IS NOT NULL THEN
            u.fixed_at - u.introduced_at
        ELSE
            NULL
        END,
        CASE WHEN u.fixed_at IS NOT NULL
            AND u.fixed_at <= u.snapshot_date THEN
            TRUE
        ELSE
            FALSE
        END,
    ...
    FROM( SELECT DISTINCT
            snapshot_date
        FROM
            vuln_daily_by_workload) d
    CROSS JOIN LATERAL vuln_upsert_data_for_date(d.snapshot_date) u
WHERE
    u.workload_id IN(
        SELECT
            id
