...
- **KEV**: known exploited vulnerabilities, merged from the CISA and ENISA catalogs (and VulnCheck KEV when enabled); `cve.kev_sources` records which catalogs have listed a CVE. KEV entries are never cleared: a CVE stays KEV even if its catalog drops it or the source fails or is disabled (hide unwanted findings with suppression). `vulnerability_summary.kev_count` is the per-image count of findings in it — a signal, not a priority tier.
